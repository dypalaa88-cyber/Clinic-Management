...
<head>
    ...
    <style>
        ...
        .btn { padding: 6px 14px; border: none; border-radius: 4px; cursor: pointer; color: #fff; font-size: 12px; text-decoration: none; }
        .btn-add { background: #198754; }
        .btn-del { background: #dc3545; }
        .btn-back { background: #6c757d; }
        input[type=search] { padding: 8px; border: 1px solid #ced4da; border-radius: 4px; font-size: 13px; width: 100%; margin-bottom: 8px; }
        .service-list { border: 1px solid #ced4da; border-radius: 4px; max-height: 200px; overflow-y: auto; padding: 8px; }
        .service-list label { display: block; padding: 4px 0; font-size: 13px; cursor: pointer; }
    </style>
</head>
<body>
    ...
    {{-- (3) جدول الخدمات الحالية --}}
    <table>
        <thead>
            <tr>
                <th>#</th><th>الخدمة</th><th>التصنيف</th><th>السعر</th><th>الإجمالي</th>
                @if(!$payment->is_locked)<th>حذف</th>@endif
            </tr>
        </thead>
        <tbody>
            @php $gt = 0; @endphp
                ...
                <tr>
                    ...
                    <td>
                        @switch($item->category)
                            @case('service')🩺 خدمة طبية@break
                            @case('medicine')💊 دواء@break
                            @case('supply')📦 مستلزم@break
                            @case('lab')🔬 تحليل@break
                            @case('radiology')🩻 أشعة@break
                            @default{{ $item->category }}
                        @endswitch
                    </td>
                    <td>{{ number_format($item->price,2) }} ج</td>
                    <td style="font-weight:bold;">{{ number_format($item->total,2) }} ج</td>
                    @if(!$payment->is_locked)
                    <td style="text-align:center;">
                        <a href="{{ route('payment.remove-item-page', ['payment' => $payment->id, 'item' => $item->id]) }}" 
                           class="btn btn-del" onclick="return confirm('تأكيد الحذف؟')">✕</a>
                    </td>
                    @endif
                </tr>
            @empty
        ...
        <tfoot>
            <tr style="background:#e9ecef;font-weight:bold;">
                <td colspan="{{ $payment->is_locked ? '4' : '5' }}" style="text-align:left;">الإجمالي الكلي</td>
                <td>{{ number_format($gt,2) }} ج</td>
                @if(!$payment->is_locked)<td></td>@endif
            </tr>
        </tfoot>
    </table>

    {{-- (4) إضافة خدمات (مع مربع بحث) --}}
    @if(!$payment->is_locked)
    ...
    <p style="font-size:12px;color:#6c757d;margin-top:10px;">
        طريقة الدفع: <strong>{{ match($payment->payment_method) { 'cash' => 'نقدي', 'card' => 'بطاقة', 'insurance' => 'تأمين', 'corporate' => 'تعاقد', default => $payment->payment_method } }}</strong> 
        | تاريخ: {{ $payment->updated_at->format('Y-m-d H:i') }}
    </p>
